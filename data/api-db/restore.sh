...
RESTORE_FOLDER=data/api-db/backup
if [ -n "$1" ]; then
	BACKUP_FILE=$1
else
...
docker exec -it fs-freelance-app-db sh -c "mongorestore -u $API_DB_USER -p $API_DB_PASSWORD --authenticationDatabase=admin --gzip --archive=$CONTAINER_BACKUP_FOLDER$backupName"
